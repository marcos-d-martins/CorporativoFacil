...
class Verificacao {
    ...
    public static function imagens($imagemComExtensao,$nome_foto,$diretorioFoto,$largura=null,$altura=null) {
        self::$dados = $diretorioFoto .'img/' . $imagemComExtensao;
        
        if(file_exists(self::$dados) && !is_dir(self::$dados)):
            $caminhoFoto = PAGINA_INICIAL;
            $imagem = self::$dados;
            //return $caminhoFoto . $imagem;
            echo "<img src=\"{$caminhoFoto}/tim.php?src={$caminhoFoto}/{$imagem}&w={$largura}&h={$altura}\" alt='{$nome_foto}' title='{$nome_foto}'>";
        else:
            echo "<hr>Arquivo não encontrado.";
            return false;
        endif;
    }
